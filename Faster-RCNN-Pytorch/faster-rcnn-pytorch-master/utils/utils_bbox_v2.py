# utils_bbox_mix.py - 支持ResNet+FPN版本的DecodeBox
...
def loc2bbox(src_bbox, loc):
    ...
    src_width  = (src_bbox[:, 2] - src_bbox[:, 0]).unsqueeze(1)
    src_height = (src_bbox[:, 3] - src_bbox[:, 1]).unsqueeze(1)
    src_ctr_x  = src_bbox[:, 0].unsqueeze(1) + 0.5 * src_width
    src_ctr_y  = src_bbox[:, 1].unsqueeze(1) + 0.5 * src_height
...
class DecodeBox:
    ...
    def forward(self, roi_cls_locs, roi_scores, rois, image_shape, input_shape, nms_iou=0.3, confidence=0.5):
        ...
        for i in range(bs):
            ...
            roi = rois[i].unsqueeze(1).expand_as(roi_cls_loc)
            cls_bbox = loc2bbox(roi.contiguous().view(-1, 4), roi_cls_loc.contiguous().view(-1, 4))
            cls_bbox = cls_bbox.view([-1, self.num_classes, 4])

            cls_bbox[..., [0, 2]] /= input_shape[1]
            cls_bbox[..., [1, 3]] /= input_shape[0]
            ...
            for c in range(1, self.num_classes):
                c_confs = prob[:, c]
                mask = c_confs > confidence

                if mask.sum() > 0:
                    boxes = cls_bbox[mask, c]
                    scores = c_confs[mask]
                    keep = nms(boxes, scores, nms_iou)

                    good_boxes = boxes[keep]
                    confs = scores[keep][:, None]
                    labels = (c - 1) * torch.ones((len(keep), 1), device=boxes.device)
            ...
            if results[-1]:
                ...
                box_xy = (results[-1][:, :2] + results[-1][:, 2:4]) / 2
                box_wh = results[-1][:, 2:4] - results[-1][:, :2]
